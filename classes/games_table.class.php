...
add_filter('set-screen-option', array('\WP_Clanwars\GamesTable', 'handle_screen_option'), 10, 3);

class GamesTable extends \WP_List_Table {

    const PER_PAGE_OPTION = 'wp_clanwars_games_per_page';
    ...
    function get_table_classes() {
        ...
        $classes[] = 'wp-clanwars-games-table';
        return $classes;
    }
    ...
    protected function handle_row_actions( $item, $column_name, $primary ) {
        ...
        $edit_link = admin_url('admin.php?page=wp-clanwars-games&amp;act=edit&amp;id=' . $item->id);
        $maps_link = admin_url('admin.php?page=wp-clanwars-games&amp;act=maps&amp;game_id=' . $item->id);
        $delete_link = wp_nonce_url('admin-post.php?action=wp-clanwars-gamesop&amp;do_action=delete&amp;items[]=' . $item->id . '&amp;_wp_http_referer=' . urlencode($_SERVER['REQUEST_URI']), 'wp-clanwars-gamesop');
        $export_link = wp_nonce_url('admin-post.php?action=wp-clanwars-gamesop&amp;do_action=export&amp;items[]=' . $item->id . '&amp;_wp_http_referer=' . urlencode($_SERVER['REQUEST_URI']), 'wp-clanwars-gamesop');
