...
class MakePOT {
	...
	function wp_clanwars($dir, $output, $slug = null) {
		$excludes = array( 'countries.php' );
		return $this->wp_plugin($dir, $output, $slug, $excludes, array());
	}

	function wp_clanwars_countries($dir, $output, $slug = null) {
		$countries_path = 'countries.php';
		$includes = array( $countries_path ); 
		$res = $this->xgettext('wp-plugin', $dir, $output, array(), array(), $includes);
		return $res;
	}

	function wp_plugin($dir, $output, $slug = null, $excludes = array(), $includes = array()) {
		$placeholders = array();
		// guess plugin slug
		if (is_null($slug)) {
		...
		$res = $this->xgettext('wp-plugin', $dir, $output, $placeholders, $excludes, $includes);
		if (!$res) return false;
		$potextmeta = new PotExtMeta;
